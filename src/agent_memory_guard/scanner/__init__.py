...
in AI agent code. Used by the CLI (`amg scan`) and the GitHub Action.
...
- Missing input validation on tool outputs stored to memory (AMG005)
- Excessive permission grants in agent memory (AMG006)
"""
...
class Severity(Enum):
    ...
    def numeric(self) -> int:
        return {"low": 1, "medium": 2, "high": 3, "critical": 4}.get(self.value, 0)
...
SECRET_PATTERNS = [
    ...
    (r"['\"](?:eyJ)[A-Za-z0-9_-]{50,}['\"]", "JWT token literal"),
]
...
INJECTION_PATTERNS = [
    ...
    (r"act\s+as\s+(?:an?\s+)?(?:admin|root|system|developer)", "Prompt injection: role escalation"),
    (r"reveal\s+(?:the\s+)?(?:system\s+)?(?:prompt|instructions)", "Prompt injection: prompt extraction"),
]
...
UNSAFE_DESER_PATTERNS = [
    ...
    (r"yaml\.(?:unsafe_)?load\s*\((?!.*Loader\s*=\s*yaml\.SafeLoader)", "Unsafe YAML load (use safe_load)"),
    ...
    (r"shelve\.open\s*\(", "Shelve uses pickle internally"),
]

# Patterns indicating missing tool output validation
TOOL_OUTPUT_PATTERNS = [
    (r"tool_output\s*=.*\n.*memory.*=.*tool_output", "Tool output stored without validation"),
    (r"result\s*=\s*tool\.(?:run|invoke|execute)\s*\(.*\n.*(?:memory|store|state).*=.*result", "Unvalidated tool result stored to memory"),
]

# Patterns indicating excessive permission grants
PERMISSION_PATTERNS = [
    (r"['\"](?:admin|root|superuser)['\"].*(?:role|permission|access)", "Excessive permission grant in memory"),
    (r"allow_all\s*=\s*True", "Unrestricted access flag"),
    (r"trust_level\s*=\s*['\"](?:full|unlimited|unrestricted)['\"]", "Unlimited trust level"),
]
...
class MemorySecurityScanner:
    ...
    def _scan_file(self, file_path: Path) -> list[Finding]:
        ...
        is_memory_file = any(re.search(pat, content) for pat in MEMORY_INDICATORS)
                        ...
                        if re.search(pattern, line):
                            findings.append(
                                Finding(
                                    ...
                                    description=(
                                        "Memory write detected without Agent Memory Guard protection. "
                                        "This operation is vulnerable to memory poisoning attacks (OWASP ASI06)."
                                    ),
                                    ...
                                    snippet=line.strip(),
                                    recommendation=(
                                        "Wrap memory operations with MemoryGuard: "
                ...
                if re.search(pattern, line, re.IGNORECASE):
                    findings.append(
                        Finding(
                            ...
                            description=(
                                f"{desc} found in source code. If stored in agent memory, "
                                "this could be exfiltrated via memory poisoning."
                            ),
                ...
                if re.search(pattern, line, re.IGNORECASE):
                    ...
                    findings.append(
                        Finding(
                            ...
                            snippet=stripped[:80],
                            recommendation=(
                                "Validate all inputs before storing in agent memory. "
                ...
                if re.search(pattern, line):
                    ...
                    findings.append(
                        Finding(
                            ...
                            description=(
                                f"{desc}. Deserializing untrusted data into agent memory "
        ...
        # Rule 5: Missing tool output validation (in memory-related files)
        if is_memory_file:
            for i, line in enumerate(lines, 1):
                for pattern, desc in TOOL_OUTPUT_PATTERNS:
                    # Multi-line check: combine current and next line
                    combined = line
                    if i < len(lines):
                        combined = line + "\n" + lines[i]
                    if re.search(pattern, combined, re.IGNORECASE):
                        findings.append(
                            Finding(
                                rule_id="AMG005",
                                title="Unvalidated tool output stored to memory",
                                description=(
                                    f"{desc}. Tool outputs should be validated before "
                                    "being stored in agent memory to prevent poisoning."
                                ),
                                severity=Severity.HIGH,
                                file_path=str(file_path),
                                line=i,
                                snippet=line.strip()[:80],
                                recommendation=(
                                    "Validate tool outputs before storing: "
                                    "`guard.write(key, tool_output, source_class=SourceClass.EXTERNAL_TOOL)`"
                                ),
                            )
                        )

        # Rule 6: Excessive permission grants
        for i, line in enumerate(lines, 1):
            for pattern, desc in PERMISSION_PATTERNS:
                if re.search(pattern, line, re.IGNORECASE):
                    findings.append(
                        Finding(
                            rule_id="AMG006",
                            title="Excessive permission grant",
                            description=(
                                f"{desc}. Overly permissive access stored in agent memory "
                                "can be exploited via privilege escalation attacks."
                            ),
                            ...
                            snippet=line.strip()[:80],
                            recommendation=(
                                "Apply least-privilege principles. Use MemoryGuard's "
                                "protected_keys policy to lock down permission-related memory."
                            ),
                        )
                    )
